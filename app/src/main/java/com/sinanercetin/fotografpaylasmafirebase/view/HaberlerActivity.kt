...
import com.sinanercetin.fotografpaylasmafirebase.adapter.HaberRecyclerAdapter
    ...
    fun verileriAl(){
        ...
        db.collection("Post").orderBy("tarih", Query.Direction.DESCENDING).addSnapshotListener { snapshot, exception ->
            if (exception != null){
                ...
                if (snapshot != null){
                    // snapshot ın başına ! geldiği zaman olumsuzluk anlamı veriyor. snapshot boş değilse demek
                    // !snapshot.isEmpty ile snapshot.isEmpty == false aynı anlama gelmektedir. snapshot'ın içi boş değilse demek
                    if (!snapshot.isEmpty){
